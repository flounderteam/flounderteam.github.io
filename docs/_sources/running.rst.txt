...
The `Zipline pipeline framework <https://flounderteam.github.io/refs/zipline/appendix.html#pipeline-api>`_
...
.. code-block:: python
    ...
    sep_pipe_loader = fsharadar.get_pipeline_loader(sep_bundle_data)
    daily_pipe_loader = fsharadar.get_pipeline_loader(daily_bundle_data)
    

Connecting Pipeline Engine with Pipeline Loaders
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Th next step selects the appropriate
PipelineEngine 
...
`SimplePipelineEngine <https://flounderteam.github.io/refs/zipline/appendix.html#zipline.pipeline.engine.SimplePipelineEngine>`_
that computes
...
`Filters <https://flounderteam.github.io/refs/zipline/appendix.html#zipline.pipeline.Filter>`_.
Designed after the popular Quantopian version,  the *fsharadar* module provides the built-in TradableStocksUS
universe that selects domestic common stocks with market cap bigger than $350 million, median daily dollar
volume greater than or equal to 2.5 million over the trailing 200 days, and closing price higher than $5 per share.
...
Defining Pipeline Custom Factors
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
`CustomFactor <https://flounderteam.github.io/refs/zipline/appendix.html#zipline.pipeline.CustomFactor>`_ class.
...
within a single
`Pipeline <https://flounderteam.github.io/refs/zipline/appendix.html#pipeline-api>`_.
